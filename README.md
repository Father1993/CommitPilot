...
-   🧠 Поддержка нескольких AI провайдеров:
    -   **AITUNNEL** (по умолчанию, модели deepseek-r1 и другие)
...
После установки настройте API токен одним из способов:
...
Ваш API токен можно хранить в файле `.env` (рекомендуется) или `config.ini`. Для безопасности:

1. **Файлы `.env` и `config.ini` уже добавлены в `.gitignore`**, чтобы предотвратить случайную публикацию ваших ключей
2. **Не публикуйте `.env` или `config.ini` с вашими реальными ключами** в публичных репозиториях
3. **Получите API токен** на:
    - **AITUNNEL** (рекомендуется): https://aitunnel.ru/
